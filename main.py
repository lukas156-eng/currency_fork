...
class Window:
    def __init__(self, root):
        ...
        self.dropbox = ctk.CTkOptionMenu(master=root,variable=self.option_var_in, values=["Euro€", "US Dollar$", "Schweizer Franken"] )   # Deine Währung
        ...
        self.dropbox = ctk.CTkOptionMenu(master=root,variable=self.option_var_out, values=["Euro€", "US Dollar$","Schweizer Franken"])  # Ausgabe währung 
    ...
    def convert(self):
        ...
        elif self.option_var_in.get() == "Euro€" and self.option_var_out.get() == "Schweizer Franken":
            user_input = self.txt_in.get()
            rechnung = float(user_input) * 0.96
            self.textvar_out.set(f"{rechnung:.2f} CHF")


        elif self.option_var_in.get() == "Schweizer Franken" and self.option_var_out.get() == "Euro€":
            user_input = self.txt_in.get()
            rechnung = float(user_input) * 1.04
            self.textvar_out.set(f"{rechnung:.2f}€")


        elif self.option_var_in.get() == "US Dollar$" and self.option_var_out.get() == "Schweizer Franken":
            user_input = self.txt_in.get()
            rechnung = float(user_input) * 0.83
            self.textvar_out.set(f"{rechnung:.2f} CHF")


        elif self.option_var_in.get() == "Schweizer Franken" and self.option_var_out.get() == "US Dollar$":
            user_input = self.txt_in.get()
            rechnung = float(user_input) * 1.20
            self.textvar_out.set(f"{rechnung:.2f}$")


        else:
            user_input = self.txt_in.get()
            self.textvar_out.set(f"{float(user_input):.2f}")
